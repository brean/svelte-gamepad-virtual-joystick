<script lang="ts">
  ...
  let { display = $bindable<boolean>(false) } = $props();
</script>
<div id="nav_modal" class="modal" style:display={display ? 'block' : 'none'}>
  <div class="modal-content">
  Configuration
  <button onclick={() => {
    display=false}}>close</button>
  <br />
      ...
      <tr>
        ...
        <td>
          {#each vb.gamepad_buttons as btn}
            <input value={btn} type="number" /><br />
          {/each}
        </td><td>
          <button>+</button>
        </td>
